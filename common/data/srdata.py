import os
import glob
from common.data import common
import pickle
import numpy as np
import imageio
import torch
...
class SRData(data.Dataset):
    ...
    def get_patch(self, lr, hr):
        args = self.args
        adjust_size_mode = args.adjust_size_mode
        scale = self.scale[self.idx_scale]
        multi_scale = len(self.scale) > 1
        if self.train:
           # print('****preparte data****')
            if adjust_size_mode == "patch":
            ...
            elif adjust_size_mode == "resize":
                lr, hr = common.resize_image(lr, hr, patch_size=args.patch_size)
            else:
                print(f"{adjust_size_mode} is not supported in {__file__}:line 218")
