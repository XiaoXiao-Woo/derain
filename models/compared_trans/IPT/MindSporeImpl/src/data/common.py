...
def get_patch(*args, patch_size=96, scale=2, input_large=False):
    ...
    ix = random.randrange(0, iw - ip + 1) #c
    iy = random.randrange(0, ih - ip + 1) #r

    if not input_large:
        tx, ty = scale * ix, scale * iy
    else:
        tx, ty = ix, iy
    #get_patch_b: (321, 481, 3) (481, 321, 3) scale: 1
    # 373 2 373 2
    # print(ix, iy, tx, ty)
    # ret = [args[0][iy:iy + ip, ix:ix + ip, :], *[a[ty:ty + tp, tx:tx + tp, :] for a in args[1:]]]
    ret = [args[0][iy:iy+ip, ix:ix+ip, :], args[1][ty:ty+tp, tx:tx+tp, :]]
