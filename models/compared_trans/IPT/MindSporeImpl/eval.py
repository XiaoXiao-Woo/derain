...
def eval_net():
    ...
    net_m = IPT(args)
    args.pth_path = "./cache/results/edsr_baseline_x2/model_12.ckpt"
    if args.pth_path:
    ...
    for batch_idx, imgs in enumerate(train_loader):
        ...
        pred = inference.forward(lr, idx)#0-1
        pred_np = add_mean(pred.asnumpy())#0-255
        pred_np = quantize(pred_np, 255)
        psnr = calc_psnr(pred_np, hr * 255., 4, 255.0)
