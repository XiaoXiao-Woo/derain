...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        self.rc = RandomResizedCrop(args.patch_size)
    ...
    def get_patch(self, lr, hr):
        ...
        if self.train:
            ...
            lr, hr = self.rc(lr, hr)
            lr, hr = np.array(lr, copy=False).astype(np.float32) / 255, np.array(hr, copy=False).astype(
                np.float32) / 255
            # lr = np.transpose(lr.astype(np.float32) / 255, (2, 0, 1))
            # hr = np.transpose(hr.astype(np.float32) / 255, (2, 0, 1))
