...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if train:
            self.repeat \
                = 1 if args.test_every // (len(self.images_hr) // args.batch_size) == 0 else args.test_every // (len(self.images_hr) // args.batch_size)

    # Below functions as used to prepare images
    def _scan(self):
        dirs = os.listdir(self.dir_hr)
        names_hr = []
        for dir in dirs:
            # name_hr = sorted(
            #     glob.glob(os.path.join(self.dir_hr+'/'+dir, '*' + self.ext[0]))
            # )
            name_hr = glob.glob(os.path.join(self.dir_hr+'\\'+dir, '*' + self.ext[0]))
            names_hr.extend(name_hr)
        names_hr = sorted(names_hr)
        # print(names_hr[0], names_hr[-1])
