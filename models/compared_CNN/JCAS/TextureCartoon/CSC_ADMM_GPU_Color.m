function [ Z  FY] = CSC_ADMM_GPU_Color( Filters, FX, MaxIter, lambda, p, rho, mu0 )
...
FR = gpuArray(zeros(M,N,C,K));
FTX = gpuArray(zeros(M,N,C,K));

Z = gpuArray(zeros(M,N,C,K));
S = gpuArray(zeros(M,N,C,K));
T = gpuArray(zeros(M,N,C,K));
