...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='Rain200H', type=str,
    ...
    gt = tf.placeholder(dtype=tf.float32, shape=[args.batch_size, args.patch_size, args.patch_size, 3])
    inputs = tf.placeholder(dtype=tf.float32, shape=[args.batch_size, args.patch_size, args.patch_size, 3])


    ######## network architecture
    outputs = model.Inference(inputs)
    ######## loss function
    l1 = tf.losses.absolute_difference(gt, outputs)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(start_epoch, args.epochs+1):
            ...
            for i, (O, B) in enumerate(train_loader):
                ###################################################################
                #### training phase! ###########################
                i += 1
                ...
                _, l1_loss = sess.run([g_optim, l1], feed_dict={gt: B / 255.0, inputs: O / 255.0}) #, all_sum
                if i % 10 == 0:
                    log_string("Epoch: " + str(epoch) + " Iter: " + str(i) + " l1: " + str(l1_loss))
            if epoch % 1 == 0 and epoch != 0:
                saver.save(sess, model_save_dir + '/' + str(epoch) + '/model-' + str(epoch) + '.ckpt')
                log_string("Save Model")

        end = datetime.datetime.now()
        log_string('time cost of DualGCN = {}s'.format(str(end - start)))