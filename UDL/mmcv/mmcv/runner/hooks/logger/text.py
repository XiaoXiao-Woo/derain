...
class TextLoggerHook(LoggerHook):
    ...
    def _log_info(self, log_dict, runner):
        ...
        if log_dict['mode'] == 'train':
            ...
            if self.by_epoch:
                log_str = f'Epoch({log_dict["mode"]}) ' \
                    f'[{log_dict["epoch"]}/{log_dict["maxepoch"]}]:[{log_dict["iter"]}]/[{log_dict["maxiter"]}]\t'
            else:
                log_str = f'Iter({log_dict["mode"]}) [{log_dict["iter"]}]: [{log_dict["inner_iter"]}/{log_dict["maxiter"]}]\t'
        ...
        for name, val in log_dict.items():
            ...
            if name in [
                    ...
                    'memory', 'epoch', 'maxepoch', 'maxiter', 'inner_iter'
            ]:
                continue
    ...
    def log(self, runner):
        ...
        log_dict = OrderedDict(
            ...
            maxepoch=runner.max_epochs,
            iter=cur_iter,
            maxiter=len(runner.data_loader))

        if not self.by_epoch:
            log_dict.update(inner_iter=runner.inner_iter)
