...
class ModelCheckpoint(Hook):
    ...
    def __init__(self, indicator: str, formatter_filename="model_best_{epoch},{best_metric}", print_freq=1, save_top_k: int=1, warmup_epoch=0,
    ...
    def before_run(self, runner):
        self.best_epoch = runner.meta.setdefault('best_epoch', 0)
        self.best_prec1 = runner.meta.setdefault('best_prec1', 0)
    ...
    def save_checkpoint(self, runner, metrics):
        ...
        if not hasattr(runner.model, 'train') and isinstance(runner.model.model, dict):
            ...
            for k, m in runner.model.model.items():
                ...
                'iter': runner.iter,
                'model': m,
                'best_metric': {name: value for name, value in metrics.items() if name not in ['grad_norm', 'lr', 'time', 'data_time']},
                # 保存多个metric的数值,  实际比较的时候还是只有一个
                'loss': metrics['loss'],
                'best_epoch': runner._epoch,
                'optimizer': runner.optimizer[k]
            }
                runner.metrics.update({'best_metric': {k: stats[k]['best_metric']}, 'best_epoch': {k: stats[k]['best_epoch']}})
        else:
            stats = {
                'epoch': runner.epoch,
                'iter': runner.iter,
        ...
        if save_top_k >= 1 and self.warmup_epoch >=0:
            ...
            if len(best_k_model) >= save_top_k:
                ...
                for index in sortedIndex_best_k_model:
                    if new_best_k_model_flag[index]:
                        ...
                        fname = os.path.join(self.save_model_path, best_k_model[index][2] + '.pth.tar')
...
if __name__ == '__main__':
    ...
    net = nn.Conv2d(3, 3, 3, 1, 1)
    ckpt = ModelCheckpoint('sam', save_top_k=1, print_freq=5, formatter_filename="{epoch}.pth.tar, {best_metric}")
    for i in range(100):
        stats2user = dict(best_metric={'sam': random.random(), 'ergas': random.random()}, epoch=i, best_epoch=i,
        arch='conv',
        ...
        # stats = dict(**stats2user, state_dict=net.state_dict())
        time.sleep(0.5)
        # print(stats)
        print(stats2user['epoch'], stats2user['best_metric'])
        # model_checkpoint(stats2user, 'score', save_top_k=5)
        ckpt(stats2user)